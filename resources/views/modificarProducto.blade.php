...
@section('contenido')
        ...
        <h1>Modificacion de un nuevo producto</h1>
        ...
        @foreach( $marcas as $marca )
                    <option value="{{ $marca->idMarca }}" {{$Producto->idMarca == $marca->idMarca ? 'selected' : null}}>{{ $marca->mkNombre }}</option>
        @endforeach
                </select>
                <br>
                Categoría: <br>
        ...
        @foreach( $categorias as $categoria)
                    <option value="{{ $categoria->idCategoria }}" {{$Producto->idCategoria == $categoria->idCategoria ? 'selected' : null}}>{{ $categoria->catNombre }}</option>
        @endforeach
                </select>
                <br>
                Presentacion: <br>
                ...
                <br>
                Imagen actual: <br>
                <img src="{{ asset('productos/') . '/' . $Producto->prdImagen }}" alt="imagen{{ $Producto->prdImagen }}" class="img-thumbnail">
                ...
                <input type="hidden" name="orgImagen" value="{{$Producto->prdImagen}}">
